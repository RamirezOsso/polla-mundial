...
export function usePredictions(userId?: string) {
  ...
  const savePrediction = async (matchId: string, homeScore: number, awayScore: number) => {
    ...
    const { data } = await supabase
      .from('predictions')
      .upsert({ user_id: userId, match_id: matchId, home_score: homeScore, away_score: awayScore }, { onConflict: 'user_id,match_id' })
      .select()
      .single()
    if (data) {
      setPredictions(prev => {
  ...
  return { predictions, loading, savePrediction, refetch: load }
}
