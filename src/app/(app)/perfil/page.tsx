...
export default function PerfilPage() {
  ...
  const handleChangePass = async () => {
    ...
    else { setPassMsg('✅ Contraseña actualizada'); setChangingPass(false); setPassForm({ current: '', new: '', confirm: '' }) }
    setSaving(false)
  }
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {userRank?.rank && (
              <p className="text-sm text-gray-400 mt-1">🏆 Puesto #{userRank.rank} · {userRank.total_points} pts</p>
            )}
            {profile.country && <p className="text-xs text-gray-500 mt-1">🌍 {profile.country}</p>}
            {profile.favorite_team && <p className="text-xs text-gray-500">❤️ {profile.favorite_team}</p>}
            <p className="text-xs text-gray-600 mt-1">
              Registrado: {new Date(profile.created_at).toLocaleDateString('es-CO')}
            </p>
          </div>
          <button onClick={() => setEditing(!editing)}
            className="text-xs border border-gray-600 hover:border-gray-400 text-gray-400 hover:text-white px-3 py-1.5 rounded-lg transition-all">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Exactos', value: profile.total_exact_scores, icon: '🎯', color: 'text-green-400' },
          { label: 'Acertados', value: profile.total_correct_results, icon: '✅', color: 'text-blue-400' },
          { label: 'Efectividad', value: `${accuracy}%`, icon: '📈', color: 'text-purple-400' },
        ].map(s => (
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
        ...
        {changingPass && (
          <div className="mt-4 space-y-3">
            {passMsg && (
              <p className={`text-sm p-2 rounded-lg ${passMsg.startsWith('✅') ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                {passMsg}
              </p>
            )}
            {[
      ...
      <button onClick={signOut}
        className="w-full py-3 border border-red-500/30 text-red-400 hover:bg-red-500/10 font-bold rounded-2xl transition-all">
        🚪 Cerrar sesión
      </button>
    </div>
  )
