...
export default function AdminUsersPage() {
  ...
  const handleToggleSpectator = async (user: any) => {
    const action = user.is_spectator ? 'quitar modo espectador a' : 'marcar como espectador a'
    if (!confirm(`¿Deseas ${action} ${user.username}?`)) return
    await createClient().from('profiles').update({ is_spectator: !user.is_spectator }).eq('id', user.id)
    setUsers(prev => prev.map(u => u.id === user.id ? { ...u, is_spectator: !u.is_spectator } : u))
    if (selected?.id === user.id) setSelected((prev: any) => ({ ...prev, is_spectator: !user.is_spectator }))
  }
  ...
  const handleDelete = async () => {
    if (!confirm(`¿ELIMINAR PERMANENTEMENTE al usuario ${selected.username}?`)) return
    setSaving(true)
  ...
  const stats = {
    ...
    spectators: users.filter(u => u.is_spectator).length,
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          ...
          { label: 'Espectadores', value: stats.spectators, icon: '👀', color: 'text-orange-500' },
        ].map(s => (
              ...
              <div key={user.id} className={`flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all ${user.is_active === false ? 'opacity-50' : ''}`}>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => openModal(user, 'detail')} title="Ver detalle"
                    className="p-2 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-all">👁️</button>
                  <button onClick={() => handleToggleSpectator(user)} title={user.is_spectator ? 'Quitar espectador' : 'Marcar espectador'}
                    className={`p-2 rounded-lg transition-all ${user.is_spectator ? 'text-orange-500 bg-orange-50 dark:bg-orange-500/10' : 'text-gray-400 hover:text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-500/10'}`}>
                    👀</button>
                  ...
                  <button onClick={() => openModal(user, 'reset')} title="Restablecer contraseña"
                    className="p-2 text-gray-400 hover:text-yellow-600 dark:hover:text-yellow-400 hover:bg-yellow-50 dark:hover:bg-yellow-500/10 rounded-lg transition-all">🔑</button>
                  <button onClick={() => handleToggleActive(user)} title={user.is_active === false ? 'Activar' : 'Inactivar'}
                    className={`p-2 rounded-lg transition-all ${user.is_active === false ? 'text-green-500 hover:bg-green-50 dark:hover:bg-green-500/10' : 'text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10'}`}>
                    {user.is_active === false ? '✅' : '🚫'}</button>
                  <button onClick={() => openModal(user, 'delete')} title="Eliminar usuario"
                    className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-all">🗑️</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)}
        title={modalType === 'reset' ? `🔑 Restablecer contraseña` : modalType === 'delete' ? `🗑️ Eliminar usuario` : `👤 ${selected?.display_name || selected?.username}`}>
        {selected && (
          <div className="space-y-4">
            {modalType === 'detail' && (
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {selected.is_spectator && <span className="text-xs bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 px-2 py-0.5 rounded-full">👀 Espectador</span>}
                    </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: 'Puntos', value: selected.global_ranking?.[0]?.total_points ?? 0, icon: '⭐' },
                    { label: 'Posición', value: selected.is_spectator ? '👀 Espectador' : `#${selected.global_ranking?.[0]?.rank ?? '?'}`, icon: '🏆' },
                  ].map(s => (
                ...
                <button onClick={() => handleToggleSpectator(selected)}
                  className={`w-full py-2.5 font-bold rounded-xl text-sm transition-all ${
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleToggleActive(selected)}
                    className={`flex-1 py-2 font-bold rounded-xl text-sm ${
            ...
            {modalType === 'reset' && (
              <div className="space-y-4">
                ...
                {msg && <p className={`text-sm p-3 rounded-xl ${msg.startsWith('✅') ? 'bg-green-50 dark:bg-green-500/10 text-green-600' : 'bg-red-50 dark:bg-red-500/10 text-red-500'}`}>{msg}</p>}
            ...
            {modalType === 'delete' && (
              <div className="space-y-4">
                <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-xl p-4">
                  ...
                  <p className="text-sm text-red-500">Eliminar a <strong>{selected.username}</strong> borrará todas sus predicciones y puntos permanentemente.</p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setShowModal(false)}
                    className="py-3 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 font-bold rounded-xl">Cancelar</button>
                  <button onClick={handleDelete} disabled={saving}
                    className="py-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl disabled:opacity-50">
