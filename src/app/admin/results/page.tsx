...
const STAGES = [
  { id: 'group', label: '🏟️ Fase de Grupos' },
  { id: 'round_of_32', label: '⚔️ Ronda de 32' },
...
function MatchResult({ match, onSave }: any) {
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 border rounded-2xl p-4 ${isFinished ? 'border-green-200 dark:border-green-500/30' : 'border-gray-200 dark:border-gray-800'}`}>
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          {match.venue && <p className="text-xs text-gray-400">{match.venue}, {match.city}</p>}
        </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-center gap-4">
          <div className="text-3xl font-black text-gray-900 dark:text-white w-14 h-14 flex items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-xl">
            {home !== '' ? home : '?'}
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors">
          ← Grupos
        </button>
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {standings.map((team: any, i: number) => (
              <div key={team.id} className={`flex items-center gap-3 px-4 py-2 ${i < 2 ? 'bg-green-50 dark:bg-green-500/5' : ''}`}>
                ...
                <span className="text-xs text-gray-400">{team.played}J</span>
                <span className="text-xs text-gray-400">{team.gd > 0 ? '+' : ''}{team.gd}</span>
...
export default function AdminResultsPage() {
  ...
  if (loading) return <div className="text-gray-500 text-center py-8">Cargando partidos...</div>

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-black text-gray-900 dark:text-white">📋 Ingresar Resultados</h1>
        ...
        <div className="space-y-3">
          {stageMatches(activeStage).length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl">
              ...
              <p className="text-gray-500">No hay partidos en esta fase aún</p>
              <p className="text-xs text-gray-400 mt-1">Usa el botón "Generar eliminatorias" en el Dashboard</p>
            </div>
          ) : (
